...
Реализовать это можно двумя способами: либо уменьшать матрицу Якоби и вектор $\mathbf{F}(\mathbf{x})$ до размерности $m$, и затем подставлять найденные решения в необходимые компоненты вектора $\Delta\mathbf{x}$, который имеет размерность $n$; либо добавлять к исходной СНУ уравнения вида $x_j - a_j = 0$ к системе, которые сделают так, что при решении СЛАУ в точке $\mathbf{a} = (a_1, a_2, \dots, a_n)$ вектор $\Delta\mathbf{x}$ в необходимых компонентах будет иметь нули.
