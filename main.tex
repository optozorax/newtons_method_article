...
Обозначим за $\hat{\mathbf{x}}$ искомое решение, за $\mathbf{x}^{[k]}$ решение на $k$-м шаге и введем ещё обозначение $\Delta\mathbf{x}^{[k]} = \hat{\mathbf{x}} - \mathbf{x}^{[k]}$.
...
\textit{Заметьте, что} данный алгоритм не зависит от размерности системы, а, следовательно, может быть применен к любой размерности исходной СНУ.
...
$\mathbb{S}$ --- это функция, преобразующая $\mathbf{F}$ к размерности $n$ в конкретной точке. Об видах этой функции будет написано далее. В случае когда размерность СНУ совпадает с размерностью решения $\mathbb{S}(\mathbf{F},\ \mathbf{x}) = \mathbf{F}$.
